...
import mysqlConnect from './database/mysql.config.js';
...
app.get('/users/:id', (request, response) => {
    ...
    mysqlConnect.query('SELECT * FROM users WHERE id = ?;',
        [id], (error, result) =>{
        response.json(result)
    })
})
...
app.post('/users', (req,res) => {
    ...
    mysqlConnect.query('INSERT INTO users (full_name, email, password) VALUES (?,?,?);',
        [name, email, password],
        (error, result) => {
            res.json(result)
        }
...
app.get('/user', (request, response) => {
    ...
    mysqlConnect.query('SELECT * FROM users WHERE email = ?;',
        [email], (error, result) =>{
        response.json(result)
    })
})
